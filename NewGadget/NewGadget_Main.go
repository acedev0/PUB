...
func GET_FUNC_PARAM_DYNAMIC(FIND_USING interface{}, RESULT_to_ASSSIGN_TO *interface{}, BE_VERBOSE bool, params ...interface{}) (bool, []PARAM_OBJ ) {
	...
	var USE_MISC_INLINE_METHOD = false
	...
	if IS_int {
		...
		// if a < 0 val was passed.. means we are returning SEVERAL structs or lists
		if INDEX_to_FIND < 0 {
			USE_MISC_INLINE_METHOD = true
		} else {
			USE_INDEX = true
		}
	...
	var INLINE_GROUP_PARAMS []interface{}
	...
	for _, obj := range ALL_PARAMS {
		...
		if USE_MISC_INLINE_METHOD {
			INLINE_GROUP_PARAMS = append(INLINE_GROUP_PARAMS, obj.PARAM_VALUE)
		}
	...
	if FOUND {
		...
		// .w e assign the value of the param to the pointer that was passed:

		if USE_MISC_INLINE_METHOD {
			*RESULT_to_ASSSIGN_TO = INLINE_GROUP_PARAMS		
		} else {
			*RESULT_to_ASSSIGN_TO = RESULT.PARAM_VALUE		
		}
